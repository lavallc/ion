...
#include <string.h>
#include "lumen.h"
#define MAC_ADDRESS 						 0x0123
#define DEVICE_NAME                          "ionDFU"                                               /**< Name of device. Will be included in the advertising data. */
#define MANUFACTURER_NAME                    "Lava"                                   /**< Manufacturer. Will be passed to Device Information Service. */
...
#define MAX_CONN_INTERVAL                    (uint16_t)(MSEC_TO_UNITS(10, UNIT_1_25_MS))             /**< Maximum acceptable connection interval (15 milliseconds). */
...
#define APP_ADV_INTERVAL                     56                                                    /**< The advertising interval (in units of 0.625 ms. This value corresponds to 25 ms). */
...
static uint32_t                            m_pkt_notif_target;                                       /**< Number of packets of firmware data to be received before transmitting the next Packet Receipt Notification to the DFU Controller. */
static uint32_t                            m_pkt_notif_target_cnt;                                   /**< Number of packets of firmware data received after sending last Packet Receipt Notification or since the receipt of a @ref BLE_DFU_PKT_RCPT_NOTIF_ENABLED event from the DFU service, which ever occurs later.*/
...
bool                                m_is_advertising               = false;                   /**< Variable to indicate if advertising is ongoing.*/
static uint8_t                           * mp_rx_buffer;
static ble_gap_addr_t new_address;
...
static void start_data_process(ble_dfu_t * p_dfu, ble_dfu_evt_t * p_evt)
{
    ...
    else
    {
        ...
        err_code = dfu_image_size_set(image_size);
				lumen_dfu_data_size = image_size;
				
    }
}


void finish_dfu_image_size_process(void) {
...
static void on_dfu_pkt_write(ble_dfu_t * p_dfu, ble_dfu_evt_t * p_evt)
{
    ...
    switch (m_pkt_type)
    {
				case PKT_TYPE_START_FULL_ERASE:
					  set_full_erase();
        case PKT_TYPE_START:
		...
		lumen_flash_progress = ((double)m_num_of_firmware_bytes_rcvd / (double)lumen_dfu_data_size);
		lumen_set_progress(lumen_flash_progress);
		lumen_leds_show();
}
    ...
    switch (p_evt->ble_dfu_evt_type)
    {
        ...
        case BLE_DFU_START:
            ...
            break;
				...
				case BLE_DFU_START_FULL_ERASE:
            m_pkt_type = PKT_TYPE_START_FULL_ERASE;
            break;
...
static void advertising_init(void)
{
	...
	//	uint8_t old_mac_address[BLE_GAP_ADDR_LEN];
	...
	/*	for (int i = 0; i < BLE_GAP_ADDR_LEN - 2; i++){
			old_mac_address[i] = new_address.addr[(BLE_GAP_ADDR_LEN - 3) -i];
		}
		adv_manuf_data_array.p_data = old_mac_address;
    adv_manuf_data_array.size = BLE_GAP_ADDR_LEN - 5;
	  adv_manuf_data.company_identifier =  (new_address.addr[BLE_GAP_ADDR_LEN - 1] << 8) | new_address.addr[BLE_GAP_ADDR_LEN - 2];
...
uint32_t dfu_transport_update_start()
{
    ...
    if (err_code != NRF_SUCCESS)
    {
		...
		//sd_ble_gap_address_get(&new_address);
    ...
    sec_params_init();
		...
		new_address.addr_type = BLE_GAP_ADDR_TYPE_RANDOM_STATIC;
		uint8_t bytes_avail;
		do {
			sd_rand_application_bytes_available_get(&bytes_avail);
		} while(bytes_avail < BLE_GAP_ADDR_LEN);
		sd_rand_application_vector_get(new_address.addr, BLE_GAP_ADDR_LEN);
		
		new_address.addr[5] |= 0xc0; // 2 MSBit must be '11' for RANDOM_STATIC address, see v4.0, Vol 3, Part C, chapter 10.8
		sd_ble_gap_address_set(&new_address);
	/*	uint32_t *ptr_packed_data = (uint32_t *)((BLE_FLASH_PAGE_END - 11) * BLE_FLASH_PAGE_SIZE + sizeof(uint32_t) + sizeof(uint32_t) + sizeof(uint32_t));
		memcpy(&ios_bond, ptr_packed_data, sizeof(bond_data_t));
		
		if (sd_getBootloader()) 
				err_code = sd_ble_gatts_sys_attr_set(m_conn_handle, ios_bond.central_attr.sys_attr, (uint16_t)ios_bond.central_attr.sys_attr_size);
		else
				err_code = sd_ble_gatts_sys_attr_set(m_conn_handle, NULL, 0);
		if (err_code == NRF_SUCCESS)
		{
