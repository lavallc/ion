...
static bool erase_everything = false;
            ...
            if (err_code != NRF_SUCCESS)
            {
                return err_code;
						 ...
						 bootloader_settings_get(&bootloader_settings);
							...
							if (erase_everything) {
								while (CODE_REGION_1_START + erase_size <= 0x38C00) {
										erase_size += 4;
									}
							}
							else {
									while (CODE_REGION_1_START + erase_size <= 0x31800) {
										erase_size += 4;
									}
								}
								erase_size -= 4; 
									err_code = pstorage_raw_clear(&m_storage_handle_app, erase_size);
									if (err_code != NRF_SUCCESS)
									{
											m_dfu_state = DFU_STATE_INIT_ERROR;
											return err_code;
