...
void CharacteristicBase::notify() {
    ...
    // we call this DIRECTLY instead of using BLE_CALL, as it will crash the device
    // if the client isn't ready to receive a notification
    sd_ble_gatts_hvx(_service->getStack()->getConnectionHandle(), &hvx_params);
}
...
Nrf51822BluetoothStack& Nrf51822BluetoothStack::rename() {
	if (!_inited) return *this;

	// set device name
	if (!_device_name.empty()) {
	   uint32_t error  = sd_ble_gap_device_name_set(&_sec_mode, (uint8_t*) _device_name.c_str(), _device_name.length());
...
Nrf51822BluetoothStack& Nrf51822BluetoothStack::startAdvertising() {
	...
	scanrsp.uuids_complete.uuid_cnt = uidCount;
	scanrsp.uuids_complete.p_uuids  = adv_uuids;

	BLE_CALL(ble_advdata_set, (&advdata, &scanrsp) );

    BLE_CALL(ble_advdata_set, (&advdata, NULL));//&scanrsp) );
...
Nrf51822BluetoothStack& Nrf51822BluetoothStack::onRadioNotificationInterrupt(uint32_t distanceUs, callback_radio_t callback) {
    ...
    uint32_t result = sd_nvic_SetPriority(SWI1_IRQn, NRF_APP_PRIORITY_HIGH);
...
void Nrf51822BluetoothStack::process_event() {
	uint32_t err_code;
	uint32_t evt_id;

	bool no_more_soc_evts = false;
	while(!no_more_soc_evts) {
		// Pull event from SOC.
		...
		if (err_code == NRF_ERROR_NOT_FOUND){
			...
			FileHandler.sys_event_handler(evt_id);
		}
	}

	bool no_more_ble_evts = false;
	uint16_t evt_len = _evt_buffer_size;
	while(!no_more_ble_evts) {

		// Pull event from SOC.
		err_code = sd_ble_evt_get(_evt_buffer, &evt_len);
		...
		if (err_code == NRF_ERROR_NOT_FOUND){
			...
			ANCSHandler.ble_evt_dispatch((ble_evt_t *)_evt_buffer);
...
void Nrf51822BluetoothStack::on_ble_evt(ble_evt_t * p_ble_evt) {
    switch (p_ble_evt->header.evt_id)
    {
        ...
        case BLE_GAP_EVT_DISCONNECTED:
            ...
            BondManager.set_device_type(0, (uint8_t*)0, 0);
            break;
...
//    ble_gap_evt_connected_t connected_evt = p_ble_evt->evt.gap_evt.params.connected;
    ...
    memcpy(&_peer_addr.addr,&p_ble_evt->evt.gap_evt.params.connected.peer_addr.addr, BLE_GAP_ADDR_LEN);
    _peer_addr.addr_type = p_ble_evt->evt.gap_evt.params.connected.peer_addr.addr_type;
    if (p_ble_evt->evt.gap_evt.params.connected.irk_match) {
    	_irk_match = true;
    	_irk_match_idx = p_ble_evt->evt.gap_evt.params.connected.irk_match_idx;
    }
    else {
    	_irk_match = false;
    	p_ble_evt->evt.gap_evt.params.connected.irk_match_idx = 0;
    }
    uint32_t error = sd_ble_gap_conn_param_update(p_ble_evt->evt.gap_evt.conn_handle, &_gap_conn_params);
    Serial.debug_print("CONERR");
    ...
    if (LumenSettings.is_leash_enabled()) {
    		PatternManager.set_leash_state(true);
    	}
...
void Nrf51822BluetoothStack::on_disconnected(ble_evt_t * p_ble_evt) {
	if (LumenSettings.is_leash_enabled()) {
		PatternManager.set_leash_state(false);
	}
